  ...
  return (
    <main className="mx-auto max-w-5xl px-6 py-10">
      <header className="mb-6">
        <Link href={`/stats/${encodeURIComponent(game.id)}`} className="text-sm text-blue-600 underline">
          ← Stats de la partie
        </Link>
        <h1 className="mt-2 text-3xl font-bold">
          Replay <span className="font-mono text-blue-600">{game.seed}</span>
        </h1>
              ...
              return (
                <li key={p.playerId}>
                  <div className="flex items-baseline justify-between text-sm">
                    ...
                    <span className="font-mono text-zinc-700">🍻 {total}</span>
                  </div>
      ...
      <section className="mt-6 rounded-2xl border border-zinc-200 bg-white p-4">
        ...
        {tail.length === 0 ? (
          <p className="mt-2 italic text-zinc-400">Aucun événement à ce point.</p>
        ) : (
              ...
              return (
                <li key={e.id} className="flex items-baseline gap-2">
                  ...
                  <span className="text-zinc-700">
                    ...
                    <span className="text-xs text-zinc-400">({e.source})</span>
                    {e.equivalence && (
                      <span className="ml-1 text-amber-700">[{e.equivalence}]</span>
                    )}
                  </span>
                </li>
              );
