...
export default async function SeasonsPage() {
  ...
  return (
    <main className="mx-auto max-w-3xl px-6 py-10">
      ...
      {seasons.length === 0 ? (
        <p className="rounded-2xl border border-zinc-200 bg-white p-6 text-center text-zinc-600 dark:border-zinc-700 dark:bg-zinc-800 dark:text-zinc-400">
          Aucune saison créée pour l'instant. Crée-en une via{' '}
          <code className="rounded bg-zinc-100 px-1 dark:bg-zinc-700">POST /api/seasons</code>.
        </p>
      ) : (
        <ul className="divide-y divide-zinc-200 rounded-2xl border border-zinc-200 bg-white dark:divide-zinc-700 dark:border-zinc-700 dark:bg-zinc-800">
          {seasons.map((s) => (
            <li key={s.id}>
              <Link
                href={`/seasons/${encodeURIComponent(s.id)}`}
                className="flex items-baseline gap-3 px-4 py-3 text-sm hover:bg-zinc-50 dark:hover:bg-zinc-700"
              >
