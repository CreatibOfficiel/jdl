...
export function Pawn({ player, isActive, cluster, isTeleport }: PawnProps) {
  ...
  return (
    <motion.g
      ...
      animate={{ opacity, x: cx, y: cy }}
      transition={{ type: 'spring', stiffness: 300, damping: 25, duration: STEP_MS / 1000 }}
    >
      {portalFlash && (
        <circle r={PAWN_RADIUS + 12} fill="none" stroke="#4361EE" strokeWidth={3} opacity={0.8}>
          <animate
            attributeName="r"
            values={`${PAWN_RADIUS + 8};${PAWN_RADIUS + 18};${PAWN_RADIUS + 8}`}
            dur="0.6s"
      ...
      {isActive && (
        <circle r={PAWN_RADIUS + 4} fill="none" stroke="#FFD60A" strokeWidth={2} opacity={0.85}>
          <animate
            attributeName="r"
      ...
      <circle r={PAWN_RADIUS} fill={player.hex} stroke="white" strokeWidth={2} />
      <text
        x={0}
        y={1}
