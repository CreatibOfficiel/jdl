    ...
    <div className="fixed inset-0 z-50 flex">
      ...
      <aside className="flex w-full max-w-md flex-col overflow-y-auto bg-white shadow-2xl">
        <header className="flex items-center justify-between border-b border-zinc-200 px-5 py-3">
          <h2 className="text-lg font-bold text-zinc-900">📊 Stats live</h2>
          <button
            type="button"
            onClick={onClose}
        ...
        <section className="px-5 py-4">
          ...
          <ul className="mt-2 space-y-2">
            {players.map((p) => {
              ...
              return (
                <li key={p.id} className="rounded-xl bg-zinc-50 p-3">
      ...
      <table className="text-xs">
        <thead>
          <tr>
            ...
            {players.map((p) => (
              <th key={p.id} className="px-2 py-1 text-zinc-500" title={p.name}>
                {p.emoji}
              </th>
            ))}
          </tr>
        ...
        <tbody>
          {players.map((from) => {
            ...
            return (
              <tr key={from.id}>
                <td className="whitespace-nowrap px-2 py-1 text-zinc-700" title={from.name}>
